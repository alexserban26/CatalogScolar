...
export class ProfesorService {
  protected resourceUrl =
    this.applicationConfigService.getEndpointFor("api/profesors");
  ...
  update(profesor: IProfesor): Observable<EntityResponseType> {
    return this.http.put<IProfesor>(
      `${this.resourceUrl}/${getProfesorIdentifier(profesor) as number}`,
      profesor,
      { observe: "response" }
    );
  }
  ...
  partialUpdate(profesor: IProfesor): Observable<EntityResponseType> {
    return this.http.patch<IProfesor>(
      `${this.resourceUrl}/${getProfesorIdentifier(profesor) as number}`,
      profesor,
      { observe: "response" }
    );
  ...
  query(req?: any): Observable<EntityArrayResponseType> {
    ...
    return this.http.get<IProfesor[]>(this.resourceUrl, {
      params: options,
      observe: "response",
    });
  }
  ...
  addProfesorToCollectionIfMissing(
    profesorCollection: IProfesor[],
    ...profesorsToCheck: (IProfesor | null | undefined)[]
  ): IProfesor[] {
    const profesors: IProfesor[] = profesorsToCheck.filter(isPresent);
    if (profesors.length > 0) {
      const profesorCollectionIdentifiers = profesorCollection.map(
        (profesorItem) => getProfesorIdentifier(profesorItem)!
      );
      const profesorsToAdd = profesors.filter((profesorItem) => {
        const profesorIdentifier = getProfesorIdentifier(profesorItem);
        if (
          profesorIdentifier == null ||
          profesorCollectionIdentifiers.includes(profesorIdentifier)
        ) {
          return false;
        }
